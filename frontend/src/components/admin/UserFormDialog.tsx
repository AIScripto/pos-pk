  ...
  return (
    ...
    >
      ...
      <FormSection title="User Information" columns={1}>
        <FormField
          label={
            <span className="flex items-center gap-1.5">
              Username
              {!user && <span className="text-[10px] font-normal text-slate-400">— auto-filled when role is selected</span>}
            </span>
          }
          required
        ...
        >
          <div className="relative">
            ...
            />
            {loadingSuggestion && (
              <Wand2 className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 animate-pulse text-orange-400" />
            )}
          </div>
        </FormField>
      ...
      <FormSection title="Role & Branch Assignment" columns={1}>
        ...
        {needsBranch && (
          <FormField
            label={<span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 text-orange-500" />Assigned Branch</span>}
            required
        ...
        {form.roleId && !needsBranch && (
          <p className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-900">
      ...
      {showPin && (
        <FormSection title="Counter PIN" columns={1}>
          {user?.hasPin && !form.pin && (
            <div className="flex items-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-400">
