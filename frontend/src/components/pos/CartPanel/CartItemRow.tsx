  ...
  return (
    <div className="px-3 py-2.5 border-b border-border/50 last:border-b-0 hover:bg-secondary/30 transition-colors group/row">
      ...
      <div className="flex items-center gap-2">
        {/* Deal indicator */}
        {isDeal && (
          <div className="w-5 h-5 rounded-md bg-warning/20 flex items-center justify-center shrink-0">
        ...
        {/* Qty stepper */}
        <div className="flex items-center gap-0.5 shrink-0 rounded-xl border border-border bg-secondary overflow-hidden shadow-2xs">
          <button
            onClick={() => onUpdateQuantity(item.id, item.quantity - 1)}
            aria-label={`Decrease quantity of ${name}`}
            className="w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center text-foreground hover:bg-primary hover:text-white active:scale-95 transition-all cursor-pointer"
          >
            <Minus className="w-4 h-4 shrink-0" />
          </button>
          <span className="font-display font-black text-sm sm:text-base text-foreground tabular-nums px-2 min-w-[28px] text-center select-none">
            {item.quantity}
          </span>
          <button
            onClick={() => onUpdateQuantity(item.id, item.quantity + 1)}
        ...
        <span className={cn(
          ...
          hasDiscount ? 'text-warning-text' : 'text-foreground'
        )}>
        ...
        <button
          ...
          className="w-7 h-7 flex items-center justify-center text-muted-foreground/70 opacity-60 group-hover/row:opacity-100 hover:text-danger-text transition-all shrink-0 cursor-pointer"
        >
      ...
      <button
        ...
        className="flex items-center gap-1 text-2xs font-display font-semibold text-muted-foreground hover:text-primary mt-1.5 transition-colors uppercase tracking-wide cursor-pointer"
      >
